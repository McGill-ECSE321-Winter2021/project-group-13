...
export default {
  ...
  methods: {
    ...
    createTechnicianHour(selectedCol,selectedTechnician) {
      ...
      var endBreakString = getEndBreak(breakTime);

      // Make sure lunch break is within technician hours
      if(startBreakString < startTimeString || endBreakString > endTimeString) {
        errorMessage = "Lunch break must be during shift";
      } else {
        AXIOS.post("/technicianhours/" + selectedTechnician.username + "?start=" + startTimeString + "&end=" + endTimeString + "&date=" + dateString)
          .then(response => {
            AXIOS.post("/workbreaks/" + response.data.id + "?start=" + startBreakString + "&end=" + endBreakString)
              .then(response2 => {
                this.getAllTechnicians();
              })
              .catch(e => {
    ...
    updateTechnicianHour(selectedTechnicianHour) {
      ...
      if(breakTime == "") {
        startBreakString = selectedTechnicianHour.workBreaks[0].startBreak;
      } else {
      ...
      if(startBreakString < startTimeString || endBreakString > endTimeString) {
        ...
        AXIOS.put("/technicianhours/" + selectedTechnicianHour.id + "?start=" + startTimeString + "&end=" + endTimeString + "&date=" + selectedTechnicianHour.date)
          .then(response => {
            AXIOS.put("/workbreaks/" + selectedTechnicianHour.workBreaks[0].id + "?start=" + startBreakString + "&end=" + endBreakString)
              .then(response2 => {
                this.getAllTechnicians();
              })
              .catch(e => {
